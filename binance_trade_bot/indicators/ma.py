...
class MA(Indicator):
    def __init__(self, depth, symbol, config, logger, timeframe="1m",
                 date_time=datetime.strptime('May 1 2022  01:00PM', '%b %d %Y %I:%M%p'), ticker=None):
        super().__init__(symbol, ticker=ticker or self, timeframe=timeframe, date_time=date_time, name="MA", config=config, logger=logger)
        self.depth = depth
    ...
    def draw(self, to_bar=10, from_bar=0, silent=False):
        ...
        self.figure.add_trace(go.Scatter(x=data_dict['timestamp'], y=data_dict['open'], mode='lines+markers', name='lines+markers'))
    ...
    def from_price(indicator, depth):
        return MA(depth, indicator.symbol, indicator.config, indicator.logger, indicator.timeframe, indicator.date_time, indicator.ticker)